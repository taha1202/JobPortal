...
//   if (!company_name || !job_title || !job_category || !company_description ||
//       !job_description || !requirement || !salary || !status || !city ||
//       !country || !state || !street || !picture) {
          ...
          function checkCategoryAndProceed() {
            ...
            db.query(ChckCategory, categoryParams, (err, reslt) => {
              ...
              if (reslt.length > 0) {
                ...
                const CategoryQuery = `INSERT INTO job_categories (category_name) VALUES (?)`;
                db.query(CategoryQuery, categoryParams, (err, reslt) => {
                  if (err) {
                    return db.rollback(() => {
                      res
